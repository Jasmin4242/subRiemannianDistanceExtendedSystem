...
assume([xc(t), yc(t), theta(t), phi1(t),...
    xt(t), yt(t), phi1t(t), gama(t)], 'real')
y = [xc; yc; theta; gama; phi1; phi1t];
assume(y(t), 'real');
Dy = diff(y,t);
% generalized velocities
syms Dphi1(t) Dgama(t)
s = [Dphi1; Dgama];
assume(s(t), 'real');
% geometrics
...
Res_v_m_P = simplify(v_m_P - v_m_P_NH);
G_y_alt_transl = simplify(-jacobian(Res_v_m_P, s_formula));
G_y_alt_transl = formula(G_y_alt_transl);

% trafo
A_Dphi_to_v = [(1/R) 0; 0 1];
%
col_Dtheta_v_omega = [(sin(gama))/(l*cos(gama)+l_t),...
    l_t/(l*cos(gama)+l_t)];
col_Dtheta_Dphi = col_Dtheta_v_omega / A_Dphi_to_v;
%
col_Dphi1t_v_omega = [1/(R*cos(gama))-(sin(gama)*tan(gama))/R + (l*cos(gama)*tan(gama)*sin(gama))/(R*(l*cos(gama)+l_t)),...
    (l*cos(gama)*tan(gama)*l_t)/(R*(l*cos(gama)+l_t))];
...
qbar_a = [f_appl_c; ell_appl_c; f_appl_t; ell_appl_t;...
    f_appl_w1; ell_appl_w1;...
    f_appl_tw1; ell_appl_tw1];
...
L_bar = [L_Transl_c; L_Rot_c; L_Transl_t; L_Rot_t;...
    L_Transl_w1; L_Rot_w1;...
    L_Transl_tw1; L_Rot_tw1];
...
syms Dphi1_ Dgama_ real
y_ = [xc_ yc_ theta_ gama_ phi1_ phi1t_]';
s_ = [Dphi1_ Dgama_]';
x_ = [y_; s_];
x = [y; s];
%
x_u_ = [x_; tau_w1; tau_gama];
% x_u = [x; tau_w1; tau_w2];
