...
% In diesem Skript wird der Bellaiche Algorithmus fuer den differentiell
% angetriebenen Roboter durchgefuehrt.
% Dabei wird ein beliebiger Punkt [x_des; y_des; theta_des] als Setpoint
% gewaehlt.
...
X2_vorher=[X2_1;X2_2;X2_3];

%% Auswerten am gewünschten Punkt
x_des=sym('x_des','real');
y_des=sym('y_des','real');
theta_des=sym('theta_des','real');
q=[x_des; y_des; theta_des; 0];
X1_vorher_q=subs(X1_vorher,[x1 x2 x3],[q(1) q(2) q(3)]);
X2_vorher_q=subs(X2_vorher,[x1 x2 x3],[q(1) q(2) q(3)]);

%Koordinatentransformation, sodass bezüglich p=zeros(6,1)
X1=simplify(subs(X1_vorher,[x1 x2 x3],[x1+q(1) x2+q(2) x3+q(3)]));
X2=subs(X2_vorher,[x1 x2 x3],[x1+q(1) x2+q(2) x3+q(3)]);
p=zeros(n,1);
X10=subs(X1,[x1 x2 x3],[p(1) p(2) p(3)]);
X20=subs(X2,[x1 x2 x3],[p(1) p(2) p(3)]);
...
X30=subs(X3,[x1 x2 x3],[p(1) p(2) p(3)]);
% X40=subs(X4,[x1 x2 x3],[p(1) p(2) p(3)]);
...
x = [x1;x2;x3];
y_von_x=simplify(A_inv_transp*x); %WICHTIGES Ergebnis: y1=.... y2=... in Abhängigkeit von x1,x2,...
y_von_x_0=subs(y_von_x,[x1 x2 x3],[p(1) p(2) p(3)]);
...
eq2=y2==y_von_x(2);
eq3=y3==y_von_x(3);
x_von_y=solve([eq1,eq2,eq3],[x1 x2 x3]); %WICHTIGES Ergebnis: x1=... x2=... in Abhängigkeit von y1,y2,y3,...

%damit die Vektorfelder X1 bis X6, geschrieben in den Koordinaten y, bestimmen
X1_y=subs(X1,[x1 x2 x3],[x_von_y.x1 x_von_y.x2 x_von_y.x3]);
X2_y=subs(X2,[x1 x2 x3],[x_von_y.x1 x_von_y.x2 x_von_y.x3]);
X3_y=subs(X3,[x1 x2 x3],[x_von_y.x1 x_von_y.x2 x_von_y.x3]);
...
for ii=1:n
    ...
    Y1_zeile=subs(y_zeile,[x1 x2 x3],[X1_y(1) X1_y(2) X1_y(3)]); 
    Y2_zeile=subs(y_zeile,[x1 x2 x3],[X2_y(1) X2_y(2) X2_y(3)]);
    Y1(ii,1)=Y1_zeile;
    Y2(ii,1)=Y2_zeile;
end
...
%% Taylor
Y1_taylor_1=taylor(cos(theta_des)-y1,[y1 y2 y3],[0 0 0],'Order', 4)